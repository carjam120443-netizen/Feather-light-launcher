...
class MainActivity : Activity() {
    ...
    override fun onResume() { super.onResume(); if (::root.isInitialized) { root.background = runCatching { WallpaperManager.getInstance(this).drawable }.getOrNull() ?: ColorDrawable(Color.rgb(248,248,248)); if (!showingDrawer) showHome() } }

    private fun buildUi() {
        root = SwipeRoot(this).apply { onSwipeUp={if(!showingDrawer)showDrawer()}; onSwipeDown={if(showingDrawer)showHome()}; orientation=LinearLayout.VERTICAL; background=runCatching{WallpaperManager.getInstance(this@MainActivity).drawable}.getOrNull()?:ColorDrawable(Color.rgb(248,248,248)) }
...
private class SwipeRoot(context:android.content.Context):LinearLayout(context){var onSwipeUp:(()->Unit)?=null;var onSwipeDown:(()->Unit)?=null;private var startX=0f;private var startY=0f;private var intercepting=false;private val slop=ViewConfiguration.get(context).scaledTouchSlop;override fun onInterceptTouchEvent(ev:MotionEvent):Boolean{when(ev.actionMasked){MotionEvent.ACTION_DOWN->{startX=ev.x;startY=ev.y;intercepting=false};MotionEvent.ACTION_MOVE->{val dx=ev.x-startX;val dy=ev.y-startY;if(kotlin.math.abs(dy)>slop&&kotlin.math.abs(dy)>kotlin.math.abs(dx)*1.15f){intercepting=true;return true}}};return intercepting||super.onInterceptTouchEvent(ev)};override fun onTouchEvent(ev:MotionEvent):Boolean{when(ev.actionMasked){MotionEvent.ACTION_UP->{val dy=ev.y-startY;if(kotlin.math.abs(dy)>=45f){if(dy<0)onSwipeUp?.invoke()else onSwipeDown?.invoke()};intercepting=false};MotionEvent.ACTION_CANCEL->intercepting=false};return true}}
