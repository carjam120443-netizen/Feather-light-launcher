...
class MainActivity : Activity() {
    ...
    private fun buildUi() {
        ...
        val overlay = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL; setPadding(20, 30, 20, 8); setBackgroundColor(Color.argb(25, 0, 0, 0)) }
    ...
    private fun showHome() {
        ...
        content.addView(TextView(this).apply { text = "Recent apps"; textSize = 14f; gravity = Gravity.CENTER; setTextColor(Color.WHITE); setShadowLayer(4f, 0f, 1f, Color.BLACK); setPadding(8, 8, 8, 2) })
        content.addView(createDock())
    ...
    private fun createDock(): View {
        val dock = LinearLayout(this).apply {
            orientation = LinearLayout.HORIZONTAL
            gravity = Gravity.CENTER
            setPadding(12, 8, 12, 8)
            setBackgroundColor(Color.argb(115, 35, 35, 35))
            elevation = 12f
        }
        val items = recentApps.values.toList().takeLast(6).reversed()
        if (items.isEmpty()) {
            dock.addView(TextView(this).apply { text = "Apps you open will appear here"; textSize = 13f; setTextColor(Color.WHITE); gravity = Gravity.CENTER; setPadding(16, 12, 16, 12) })
        } else items.forEach { dock.addView(createDockAppView(it)) }
        return dock
    }

    private fun createDockAppView(app: AppInfo): View {
        return ImageView(this).apply {
            setImageDrawable(app.icon); scaleType = ImageView.ScaleType.CENTER_INSIDE; contentDescription = app.label; isClickable = true
            setPadding(8, 6, 8, 6); setOnClickListener { launchApp(app) }
        }.also { it.layoutParams = LinearLayout.LayoutParams(64, 64) }
    }
    ...
    private fun launchApp(app: AppInfo) {
        recentApps.remove("${app.packageName}/${app.activityName}")
        recentApps["${app.packageName}/${app.activityName}"] = app
